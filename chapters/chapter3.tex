...
\end{align*}
If $\underline{d}(A)=\ol{d}(A)$ we call the common value $d(A)$ the (\textit{asymptotic}) \textit{density} of $A$. Note that, when $A$ has asymptotic density, we have that $d(A)\in[0,1]$. On the other hand, analogously to Riemann's Rearrangement Theorem, we can prove the following theorem:
...
\begin{proof}
    \textcolor{laser_eyes}{Não sei porque eu não completei isso ainda, é um corolário direto do lema anterior}
\end{proof}

\section{Characterization of \texorpdfstring{$\mf{dd}$}{dd}}

The main theorem of this subsection is the characterization $\mf{dd}=\textbf{non}(\mc{M})$, where the latter is a well known cardinal characteristic. To prove this using GT, we will find relational systems $\mc{A}_1$, $\mc{A}_2$ and $\mc{B}$ s.t. $\mc{A}_1\leq_T\mc{B}\leq_T\mc{A}_2$ and $\mf{d}(\mc{A}_1)=\mf{d}(\mc{A}_2)=\textbf{non}(\mc{M})$ and $\mf{d}(\mc{B})=\mf{dd}$.

Its well known that $\mc{A}_2=(\mc{M},2^\omega,\not\ni)$ works, where $\mc{M}$ are the meager sets of $2^\omega$. And, for $\mf{dd}$, we can take $\mc{B}=(D,\sym,R)$, where $D$ are the infinite-coinfinite substets of $\omega$ with density, and $xRp$ if $d(x)\neq d(p(x))$, as expected.

We will first prove the easier inequality $\mc{B}\leq_T\mc{A}_2$, you should try to find a reduction for $\mc{A}_2\leq_T\mc{B}$, you will see that's not too easy (in fact it's impossible), this is why we will later find other relational system $\mc{A}_1$ to work with the same dominating number.

\begin{lemma}{}{}\label{dd<=non(M)}
    $$(D,\sym,R)\leq_T(\mc{M},2^\omega,\not\ni)$$
\end{lemma}

\textcolor{laser_eyes}{Aqui usamos que $\omega^\omega$ é homeomorfo a $\sym$ em $2^\omega$}
...
\begin{proof}
    ...
    \end{align*}
    as we saw in the proof of theorem \ref{rr<=non(M)}, to show that $\varphi_-(A)\in\mc{M}$, it's sufficient to prove that $\varphi_-(A)^c$ is the intersection of countably many dense sets, so we will show that each $A_{k,\ell}$ is open and dense. If $p\in A_{k,\ell}$, lets $m=\max\{n_0,n_1\}$, then, for $q\in\mc{N}(p\restriction_{m+1})$, clearly $d_{n_0}(q(A))=d_{n_0}(p(A))<\frac1k$ and $d_{n_1}(q(A))=d_{n_1}(p(A))>1-\frac1k$, so $q\in A_{k,\ell}$, and then $\mc{N}(p\restriction_{m+1})\subseteq A_{k,\ell}$, so it's open.
...
\begin{definition}{}{}
    Let $h\in\omega^\omega$ with $\lvert h(n)\rvert\geq1$, $\forall n\in\omega$. A function $\phi:\omega\to[\omega]^{<\omega}$ is an $h$-slalom if $\lvert\phi(n)\rvert\leq h(n)$, $\forall n\in\omega$. We denote by $\Phi_h$ the collection of all $h$-slaloms.
\end{definition}

Now, to prove the other tukey inequality, let's take $\mc{A}_1=(\omega^\omega,\Phi_h,\in^\infty)$, where $f\in^\infty\phi$ if $f(n)\in\phi(n)$ for infinitely many $n$, and use a result due to Bartoszyński (Combinatorial aspects of measure and category):

\begin{theorem}{}{}\textbf{(Bartoszyński)} For any $h\in\omega^\omega$ satisfying $\lvert h(n)\rvert\geq1$, $\forall n\in\omega$
    \begin{align*}
...
It's possible to show that $\rp{\omega^\omega,\Phi_h,\in^\infty}$ and $\rp{\mc{M},2^\omega,\not\ni}$ are not Tukey equivalent, so the approach to prove $\mf{dd}\geq\textbf{non}(\mc{M})$ changes a lot from the proof of Lemma \ref{dd<=non(M)} and it's way trickier.

\begin{lemma}{}{} For any $h\in\omega^\omega$ growing fast enough (as equation (5) in the sketch below)
...
\begin{definition}{}{}
    $\mf{rr}$ is the smallest cardinality of any family $C\subseteq\sym$ such that, for every p.c.c. series $\sum_n a_n$ of reals, there is some $p\in C$ for which $\sum_n a_{p(n)}$ no longer converges to the same limit.
\end{definition}
...
\begin{proof}
    ...
    $$c_k=\begin{cases}
        ...
        -\frac1n\text{, if }k=a_n
    \end{cases}$$
    clearly $\sum_k c_k$ is p.c.c. and, since $a_n\geq2^{n-1}$, we have $\sum_kc_k=\infty$. Also, since by our choice of $h$, we have $p(a_{n+2})\geq2^n$, then $\sum_kc_{p(k)}=\infty$ for all $p$ (\textcolor{laser_eyes}{por que??})
\end{proof}
...
\begin{definition}{}{}
    Let $X,Y\subseteq\textbf{all}=[0,1]\cup\{\textbf{osc}\}$ where $X\neq\emptyset$ and for all $x\in X$ there is some $y\in Y$ such that $y\neq x$. The $(X,Y)$\textit{-density number} $\mf{dd}_{X,Y}$ is the smallest cardinality of a family $C\subseteq\sym$ such that for every infinite-coinfinite set $A\subseteq\omega$ with $d(A)\in X$, there is some $p\in C$ such that $d(p[A])\in Y$ and $d(p[A])\neq d(A)$.
\end{definition}
...
\begin{theorem}{}{}\label{non(M)_general}
    The following hold:
    \begin{enumerate}
        \item If $\textbf{osc}\notin X$ and $\textbf{osc}\in Y$, then
...
\begin{proof}
    1. Is easy to see that follows from Lemma \ref{dd<=non(M)} since, if $\textbf{osc}\notin X$ and $\textbf{osc}\in Y$, we can repeat the construction in the proof and find some $p\in\sym$ such that $d(p[A])=\textbf{osc}\in Y\setminus X$.

    2. \textcolor{laser_eyes}{Eu não consigo ver como alterar a construção de forma que $d(\varphi_-(g))=1$ ao invés de $0$.}
\end{proof}
...
\begin{proof}
    The case when $0\in X$ or $1\in X$ can be reduced to the previous theorem if we could prove thar $(2^\omega,\mc{N},\in)\cong(\omega^\omega,\mc{N},\in)\leq_T(\omega^\omega,\Phi_h,\in^\infty)$, which is what we will gonna prove:

    Consider the product measure $\mu=\prod_n\mu_n$ on $\omega^\omega$, where $\mu_n$ is a measure on $\omega$ defined as follows: fix a series $\sum_ka_k^n=1$ such that $0<a_k^n<\frac1{2^{n+1}h(n)}$, for all $k\in\omega$. Since $n$ is fixed, we can easily find such series. Now, given $A\subseteq\omega$, let $\mu_n(A)=\sum_{k\in A}a_k^n$.

    \textcolor{laser_eyes}{``With such measure on $\omega^\omega$ we''...???}

    Let then $0,1\notin X$ and $r\in(0,1)\cap X$.
    \textcolor{laser_eyes}{Pendente}
\end{proof}

It's worth noting that it's an open question if the previous theorem holds if $X=\{\textbf{osc}\}$.

\begin{corollary}{}{}
    If $X\cap[0,1]\neq\emptyset$, then $\textbf{cov}(\mc{N})\leq\mf{dd}_{X,Y}$
\end{corollary}

we can also compare $\mf{dd}_{X,Y}$ with $\mf{b}$ and $\mf{d}$ when $\textbf{osc}\in X$ or $\textbf{osc}\notin Y$ as follows

\begin{theorem}{}{}
    If $\textbf{osc}\in X$ or $\textbf{osc}\notin Y$, then
...
\begin{proof}
    ...
    \noindent\rule{16.6cm}{0.4pt}
        ...
        Let's now analyze the general case. If $0\in X$ or $1\in X$, the result follows \todo{and Cichoń's Diagram?}by Theorem \ref{non(M)_general}. Let's then assume $X\subseteq(0,1)$. Given $r\in(0,1)$, let:
    \begin{itemize}
...
Consider the simple relational system $(\mf{c},[\mf{c}]^{\aleph_0},\in)$, let's first prove some properties about it.
...
\begin{lemma}{}{}\label{almost_disjoint_family_d=osc}
...
\begin{proof}
    ...
    For the second case, let $0\notin Y$ (when $1\notin Y$ just take complements as in $1\in X$). Let, by lemma \ref{almost_disjoint_family_d=osc}, $\{\varphi_-(\alpha):\alpha<\mf{c}\}$ be an almost disjoint family of subsets of $\omega$ without density. Then
