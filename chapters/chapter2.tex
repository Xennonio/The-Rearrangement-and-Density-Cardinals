...
\begin{theorem}{Riemann Rearrangement}{riemann_rearrangement}
...
\end{theorem}
One natural generalization one can think about is: how much smaller could we choose $\sym$? It turns out that we can define this in terms of cardinality as we will see, but it's far from trivial what this cardinal should be. We will prove this is a cardinal characteristic of the continuum, but it is still open if we can prove the equality with any expression involving well-known cardinal characteristic in the literature.

\section{Definitions}

\begin{definition}{Rearrangement Number}{rearrangement_number}
    The \lasereye{Rearrangement Number}, $\mf{rr}$, is the smallest cardinality of any family $C\subseteq\sym$ such that, for every c.c. series $\sum_n a_n$ of reals, there is some $p\in C$ for which $\sum_n a_{p(n)}$ no longer converges to the same limit\footnote{Which includes diverging for $\pm\infty$ or oscillating}.
\end{definition}
...
Its easy to see that, if $C\subseteq\sym$ satisfies, for example, $\mf{rr}_i$, then it also satisfies $\mf{rr}_{fi}$, so $\mf{rr}_{fi}\leq\mf{rr}_i$, in general, we can make the Hasse Diagram in \cref{fig:hasse_rearrangement}.

\begin{figure}[ht]
\centering
\input{diagrams/hasse-rr}
\caption{Hasse Diagram of the Rearrangement Numbers}
\label{fig:hasse_rearrangement}
\end{figure}

using this notation, $\mf{rr}$ would be $\mf{rr}_{ifo}$, as expected.
...
Indeed, to prove that $\lvert\sym\rvert=\mf{c}$, note that $\sym\subseteq\omega^\omega$, therefore $\lvert\sym\rvert\leq\aleph_0^{\aleph_0}=2^{\aleph_0}$, so we can construct an injection $\varphi:[0,1]\to\sym$, such that for each real number $a=(0.d_0d_1d_2\dots)_2\in[0,1]$ we associate $\varphi(a)$ as the bijection in $\omega$ that swaps $2n$ with $2n+1$ if $d_n=1$, and does not swap them if $d_n=0$.

In this subsection we will prove its actually a cardinal characteristic and we will also explain an important technique about padding with zeros, which will be generalized later.

\begin{theorem}{(\cite{blass})}{rr_uncountable}
    \begin{center}
...
\begin{proof}
    Assume by contradiction that there is a $C=\{p_n:n\in\omega\}\subseteq\sym$, Let $b_n:=\frac{(-1)^n}{n}$ (indeed, we can take any term of a c.c. series). \todo{add my asymptote pic}Lets define $\ell:\omega\to\omega$ a fast growing function that will indicate the new position of $b_k$ and will add a lot of $0$'s between the $b_k$'s, i.e.
    $$a_n:=\begin{cases}
        b_k,\, n=\ell(k)\\
        0,\, \text{otherwise}
    \end{cases}$$
...
\begin{lemma}{(\cite{blass})}{mix_aux}
...
\begin{proof} Let $g_0=\emptyset$. If $g_{2k-1}:n_{2k-1}\to\omega$ was already defined, by the previous lemma there exists a $g_{2k}:n_{2k}\to\omega$ extending $g_{2k-1}$ s.t. $\text{Im}(g_{2k})=p[n_{2k}]$. Analogously, if $g_{2k}$ was already defined, let $g_{2k+1}:n_{2k+1}\to\omega$ extending $g_{2k}$ s.t. $\text{Im}(g_{2k+1})=q[n_{2k+1}]$.
...
\begin{theorem}{(\cite{blass})}{rr_narrow}
...
\begin{proof}
    ...
    Let $C\subseteq\sym$ be as in the definition of $\mf{rr}$, by The Mix Lemma, for each $p\in C$ there is a $g_p\in\sym$ s.t. $g_p$ agrees with $p$ and $q=\text{id}$ for infinitely many $n$'s. Define
    $$C':=C\cup\{g_p:p\in C\}$$
...
\end{proof}
The previous theorem narrows down the number of rearrangement cardinals (Figure \cref{fig:rearrangement_narrowed})

\begin{figure}[ht]
\centering
\input{diagrams/narhasse-rr}
\caption{Narrowed Hasse Diagram}
\label{fig:rearrangement_narrowed}
\end{figure}

\section{Measure and Category}

In this subsection we relate the rearrangement number with a lot of category cardinal characteristics.

\subsection{Rademacher's Theorem and \texorpdfstring{$\mathbf{cov}(\mc{N})$}{covN}}

In order to relate them we will need a consequence of Kolmogorov's Two Series Theorem, whose proof depends on the following inequality also due to Kolmogorov:

\begin{lemma}{Kolmogorov's Inequality}{kolmogorovs_inequality}
    Let $X_1,X_2,\dots,X_n$ be independent random variables with $\mbb{E}[X_k]=0$ and $\text{var}(X_k)<\infty$ for $k=1,\dots,n$. Then, $\forall\varepsilon>0$ we have that:
...
\begin{proof}
    ...
    \begin{equation}\label{eq:var(S_n)}
    ...
    \end{equation}
    now, substituting \cref{eq:E(S2)} in \cref{eq:var(S_n)}, we conclude that
    $$\text{var}(S_n)\geq\sum_{k=1}^n\mbb{E}[S_n^2\cdot\boldsymbol{1}_{E_k}]\geq\sum_{k=1}^n\mbb{E}[S_k^2\cdot\boldsymbol{1}_{E_k}]$$
    and, by definition, $\lvert S_k(\omega)\rvert\geq\varepsilon$ whenever $\boldsymbol{1}_{E_k}(\omega)$, therefore $\lvert S_k^2(\omega)\rvert\geq\varepsilon^2$ and, by \cref{eq:P(E)}, we have that
...
\begin{lemma}{Kolmogorov's Two Series\footnote{He also has his three series theorem}}{kolmogorovs_two_series}
    Let $(X_n)_{n\geq0}$ be independent random variables. If $\sum_n\mbb{E}[X_n]$ and $\sum_n\text{var}(X_n)$ both converges, then $\sum_n X_n$ converges almost surely.
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation}\label{eq:kolmogorov_S_n}
    ...
    and, since $E_N=\max_{m,n\geq N}\lvert S_n-S_m\rvert>\varepsilon$, by \cref{eq:kolmogorov_S_n} we have that
...
\textcolor{laser_eyes}{Como usaremos somente $c_n=\frac1n$ na prova do teorema 2.5., e sabemos que a harmônica se relaciona com a densidade assintótica por meio do ln, talvez fosse possível pular esse teorema}

\begin{theorem}{Rademacher}{rademacher}
    Let $(c_n)_{n\geq0}$ be any sequence of reals. And let
...
\begin{theorem}{(\cite{blass})}{cov_rr}
...
\begin{proof}
    ...
    has measure zero\footnote{Note here we're using $\cov{\mc{N}}$ is the same in the Cantor Space $2^\omega$ as in $\mbb{R}$}. And, since $s\mapsto s\circ p$ \todo{why?}is measure preserving in the Cantor Space $2^\omega$, then $B_p=p[A_p]$ also has measure zero.
...
\subsection{\texorpdfstring{$\mathbf{non}(\mc{M})$}{nonM} and \texorpdfstring{$\mathbf{cov}(\mc{M})$}{covM}}
...
\begin{lemma}{(\cite{blass})}{sym_hom_ww}
    The subspace $\sym$ of $\omega^\omega$ of all permutations of $\omega$ is homeomorphic to $\omega^\omega$
\end{lemma}

As mentioned in \cite{blass}, there are well known theorems that guarantees this is true, but if you want, they construct a direct homeomorphism between these spaces, but I will not add it here.

\begin{corollary}{}{non_eq}
...
In fact, it's not hard to see this is true for any other ideal cardinal characteristics, since all of them just use information of the topology. Let's now see one of the main results of this section.

\begin{theorem}{(\cite{blass})}{rr_non}
...
\begin{proof}
    ...
    we need to show that $C\cap X(a_n)\neq\emptyset$, then, its sufficient to show that $X(a_n)$ is comeager, since, if it was empty, then $C\subseteq X(a_n)^c$, but, by \cref{lem:comeager}, $C$ would also be meager.
...
\begin{theorem}{(\cite{blass})}{rrfi_cov}
...
\begin{proof}
    ...
    Then, for any c.c. series $\sum_n a_n$, let $p\in C$ be such that $\sum_n a_{p(n)}$ converges to a different number or diverges to infinity. Since $g\in A_p$, $g[M]=p[M]$ for infinitely many $M$, and also $g[M]=M$ for infinitely many, since $\text{id}\in C$. As a result, the same argument as in \cref{th:rr_narrow} shows that $\sum_n a_{g(n)}$ diverges by oscillation, so $\{g\}$ witnesses that $\mf{rr}_o=1$, a contradiction, since it's uncountable.
\end{proof}
...
\begin{definition}{Jumbling Number}{jumbling_number} Let $A\subseteq\omega$
    \begin{itemize}
        \item $A$ is \lasereye{preserved} by $p\in\sym$ if $p$ doesn't change the relative order of members of $A$, except for finitely many elements. If $A$ is not preserved by $p$ we say it's jumbled by $p$.
        \item A \lasereye{conserving family} is a family of permutations such that there is an infinite set $A$ that is preserved by every $p$ in the family.
        \item The \lasereye{jumbling number} $\mf{j}$ is the greatest cardinality of a conserving family.
    \end{itemize}
\end{definition}
Note that, if $S=\{q_n:n\in\omega\}\subseteq\sym$, let $C=\{p_m=q_m^{-1}:m\in\omega\}$, then, by doing the construction of the fast growing function $\ell:\omega\to\omega$ as in \cref{th:rr_uncountable}, we guarantee that $p_m(\ell(m))<p_m(\ell(m+1))<\dots$, which means $A=\{\ell(n):n\in\omega\}$ is not jumbled by any $p\in C$, then, since $\aleph_0<\mf{j}\leq\lvert\sym\rvert=\mf{c}$, it's a cardinal characteristic of the continuum. This proof is not necessary, since \cref{th:j_b} will guarantee that (but the argument is way harder. way more powerful, tho).

Roughly, $\mf{j}$ is defined to be the largest cardinal for which the argument in \cref{th:rr_uncountable} remains valid. Just consider that for any fast growing such function $\ell$ we can associate the set $A=\{\ell(1),\ell(2),\dots\}$ and vice-versa. We will see how to use this in the proof of the next Theorem.

\begin{lemma}{(\cite{blass})}{j_rr}
    $$\mf{j}\leq\mf{rr}$$
\end{lemma}
...
In order to bound $\mf{rr}$ with $\mf{b}$, we will prove the following theorem

\begin{theorem}{(\cite{blass})}{j_b}
    \begin{center}
...
\begin{figure}[ht]
    \centering
    \input{diagrams/p-arrows-sketch}
...
\begin{theorem}{(\cite{blass})}{d_rrfi}
    $$\mf{d}\leq\mf{rr}_{fi}$$
\end{theorem}
